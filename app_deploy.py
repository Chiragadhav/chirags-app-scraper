...
class ReviewScraper:
    ...
    def scrape_google_play_reviews(self, app_id, max_reviews=500):
        ...
        try:
            ...
            logger.error("google-play-scraper not available - install it in requirements.txt")
            # Return demo data for testing
            reviews = [
                {
                    'app_name': f'Sample App {app_id}',
                    'reviewer_name': 'Demo User',
                    'rating': 5,
                    'review_text': 'Great app! Works perfectly.',
    ...
    def scrape_app_store_reviews(self, app_id, max_reviews=500):
        ...
        try:
            ...
            logger.error("app-store-scraper not available - install it in requirements.txt")
            # Return demo data for testing
            reviews = [
                {
                    'app_name': f'Sample App {app_id}',
                    'reviewer_name': 'Demo User',
                    'rating': 4,
                    'review_text': 'Nice app with good features.',
                    'review_date': datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
                    'helpful_count': 0,
                    'platform': 'Apple App Store'
                }
...
def scrape():
    try:
        ...
        app_name = reviews[0]['app_name'].replace(' ', '_').replace('/', '_')
        filename = f"reviews_{app_name}_{timestamp}.csv"

        # Create CSV in temporary directory for cloud deployment
        df = pd.DataFrame(reviews)

        # Store CSV data in session or temporary storage
        csv_data = df.to_csv(index=False)

        # For demo purposes, store in temporary file
        import tempfile
        ...
        return jsonify({
            ...
            'csv_data': csv_data  # Include CSV data for download
        })
...
def download_file(filename):
    try:
        # For cloud deployment, use temporary directory
        import tempfile
        temp_dir = tempfile.gettempdir()
        filepath = os.path.join(temp_dir, filename)
...
@app.route('/health')
def health():
    return jsonify({'status': 'healthy', 'message': 'Chirag\'s App Scraper is running!'})
